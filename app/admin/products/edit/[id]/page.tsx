...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            ...
            setFormData({
              ...
              relatedProducts: product.relatedProductIds ? product.relatedProductIds.split(',').filter((id: string) => id) : [],
              crossSellProducts: product.crossSellProductIds ? product.crossSellProductIds.split(',').filter((id: string) => id) : [],
            });
          }
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent, isDraft: boolean = false) => {
    ...
    try {
      // Prepare categoryId - send null if no valid category selected
      ...
      // Prepare product data for API - matching backend UpdateProductCommand structure
      ...
      if (response.ok) {
        try {
          ...
          alert(isDraft ? 'Product saved as draft!' : 'Product updated successfully!');
          router.push('/admin/products');
        } catch (err) {
          console.log('Product updated (no JSON response)');
          alert(isDraft ? 'Product saved as draft!' : 'Product updated successfully!');
          router.push('/admin/products');
        }
      } else {
        try {
          ...
          try {
            ...
            if (errorJson.errors) {
              ...
              alert('Error updating product: ' + (errorJson.message || errorJson.title || 'Unknown error'));
            }
          } catch {
            alert('Error updating product: ' + (errorText || `Status: ${response.status}`));
          }
        } catch (err) {
          alert('Error updating product: Status ' + response.status);
        }
      }
    } catch (error) {
